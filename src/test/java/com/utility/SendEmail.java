...
public class SendEmail {
	...
	private static String reportFileName ="Testing Report_"+ new SimpleDateFormat("yyyyMMddHHmmss").format(new Date().getTime());
...
//		zip.zip(new File("target/surefire-reports/html"), new File("ReportZip/TestReport/"+reportFileName+".zip"));
		zip.zip(new File("test-output"), new File("ReportZip/TestReport/"+reportFileName+".zip"));
	}
	@Test(priority = 1)
	    public static void execute() throws Exception
	    {
	    	...
	    	try{
	    		...
	    		SendEmail.sendMail(emailId,	password, hostName, port, "true", "true", true, javax.net.ssl.SSLSocketFactory.class.getCanonicalName(), 
	        					"false", to, cc, bcc, "Automation Test Report","Hi \n Here is your test report of current run that was initiated.",
	                            path, reportFileName); 
	        	}  catch(Exception e) {
	             e.printStackTrace();
	            }
